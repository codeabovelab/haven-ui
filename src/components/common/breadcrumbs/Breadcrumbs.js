...
export class Breadcrumbs extends React.Component {
  ...
  _processRoute(route, routesLength, crumbsLength, isRoot, createElement) {
    ...
    if (name && 'excludes' in this.props && this.props.excludes.some(item => item === name)) return null;
    ...
    if (name) {
      ...
      return !createElement ? link : React.createElement(this.props.itemElement, { className: itemClass, key: Math.random() * 100 }, link, separator);
    }
  ...
  _buildRoutes(routes, createElement) {
    ...
    routesWithExclude.map((route, index) => {
    ...
    return !createElement ? crumbs : React.createElement(this.props.wrapperElement, {className: this.props.customClass || this.props.wrapperClass}, crumbs);
  }
...
Breadcrumbs.defaultProps = {
  ...
  wrapperElement: "ul",
  wrapperClass: "breadcrumb al-breadcrumb",
  itemElement: "li",
  itemClass: "",
  activeItemClass: "",
  excludes: [''],
